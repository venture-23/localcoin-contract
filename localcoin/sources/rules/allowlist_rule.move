...
/// A simple allowlist rule - allows only the recipients on the recipientlist to
/// perform an Action.
module localcoin::allowlist_rule {
    ...
    /// The `sender` or `recipient` is not on the merchantlist or recipientlist.
    ...
    /// Removes records from the `allowlist_rule` for a given action. The Policy
